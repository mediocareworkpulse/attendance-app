...
def edit_employee(eid):
    data = {'full_name':request.form.get('full_name','').strip(),'department':request.form.get('department','').strip(),'branch':request.form.get('branch','').strip(),'role':request.form.get('role','Staff').strip(),'password':request.form.get('password','1234').strip(),'shift_start':request.form.get('shift_start','08:00').strip(),'shift_end':request.form.get('shift_end','17:00').strip(),'updated_at':now_eat().isoformat()}
...
def add_branch():
    ...
    if n: supabase.table('branches').insert({'name':n,'shift_start':request.form.get('shift_start','08:00'),'shift_end':request.form.get('shift_end','17:00')}).execute()
...
def check_in_page():
    ...
    return render_template('check_in.html',records=records,user_status=current_status,today=today,company=COMPANY_NAME,check_in_time=check_in_time,shift_start=shift_start,shift_end=shift_end,lunch_active=lunch_active,emp_info=emp_info)

@app.route('/check-in', methods=['POST'])
@login_required
def process_attendance():
    if session.get('role') in NO_CHECKIN_ROLES: return redirect('/')
    un = session.get('user'); action = request.form.get('action')
    today = str(now_eat().date()); now = now_eat().strftime('%H:%M:%S')
    ...
    dept = emp[0].get('department',''); branch = emp[0].get('branch',''); shift_start = emp[0].get('shift_start','09:00')
        ...
        try:
            ...
            if emp and total > 0:
                if stype == 'branch_total' and role == 'Branch Manager':
                    supabase.table('branch_sales').upsert({'branch':ub,'date':today,'mpesa_sales':mpesa,'cash_sales':cash,'total_sales':total,'submitted_by':un,'notes':notes,'expenses':expenses}).execute()
                else:
...
def reports():
    ...
    if rt == 'attendance':
        for rec in safe_data(execute_query(supabase.table('attendance').select('*').gte('date',fd).lte('date',td).order('date',desc=True).limit(200))):
            st = rec.get('status','present'); records.append({'date':rec.get('date',''),'full_name':rec.get('full_name',''),'check_in':rec.get('check_in','—'),'check_out':rec.get('check_out','—'),'status':st,'label':{'present':'Working','late':'Working','lunch':'At Lunch','checked_out':'Checked Out'}.get(st,st)})
    elif rt == 'sales':
